...
def unit_risk_score(config: DictConfig) -> None:
    ...
    # Assign Kedro feature tables; strip any pre-existing s__* columns so that
    # make_global_score starts from a clean slate.
    survey_class._df_item = df_item_kedro.drop(
        columns=[c for c in df_item_kedro.columns if c.startswith('s__')]
    )
    survey_class._df_unit = df_unit_kedro.drop(
        columns=[c for c in df_unit_kedro.columns if c.startswith('s__')]
    )
    ...
    if 'survey_name' not in survey_class._df_unit.columns:
        survey_class._df_unit['survey_name'] = SURVEY
    if 'survey_version' not in survey_class._df_unit.columns:
        survey_class._df_unit['survey_version'] = 'latest'

    # Build _df_resp from unique responsibles present in unit features
    survey_class._df_resp = (
        df_unit_kedro[['responsible']]
        .drop_duplicates()
    ...
    survey_class.get_feature_item__answer_removed = lambda feature_name: df_removed_kedro.copy()

    try:
        print("Calculating Legacy Risk Scores from Kedro feature outputs...")
        ...
        if not survey_class._score_columns:
            print("No score columns with sufficient variance — cannot compute global score for this survey.")
            return

        # Determine whether the responsible-level score has enough variance to run PCA.
        # make_responsible_score receives restricted_columns=_score_columns (unit-level), so
        # only columns in _df_resp that are NOT in _score_columns are used. If they are all
        # constant after fillna(0) the StandardScaler raises "at least one array or dtype".
        ...
        print("Collecting item-level scores...")
        ...
        df_item_scores.to_parquet(item_scores_parquet, index=False)
        print(f"saved legacy item scores to {item_scores_parquet}")

        # Persist unit risk scores
        ...
        # Build merged score table (unit + responsible scores)
        ...
        id_cols    = [c for c in ['interview__id', 'responsible', 'survey_name', 'survey_version']
                      if c in df_scores.columns]
        final_cols = id_cols + ['unit_risk_score', 'responsible_score'] + sorted(score_cols)
        df_scores  = df_scores[[c for c in final_cols if c in df_scores.columns]]
        ...
        return  # do not fall through to the regular UnitDataProcessing block
