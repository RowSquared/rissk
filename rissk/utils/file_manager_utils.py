...
def fs_isdir(path, key=None, secret=None, is_local=True):
    """
...
def fs_exists(path, key=None, secret=None, is_local=True):
    """
    Check if a file or directory exists at the specified path.
...
def fs_mkrdir(path, key=None, secret=None, is_local=True):
    if is_local is True:
...
def fs_listdir(path, key=None, secret=None, is_local=True):
    if is_local is True:
...
def fs_open(file_path, key=None, secret=None, mode='r', is_local=True):
    if is_local is True:
